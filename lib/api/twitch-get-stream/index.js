    ...
    request
      ...
      .end(function(err, res) {
        ...
        if (!res.ok)
          return reject(
            new Error(
              'Could not access the twitch API to get the access token, maybe your internet or twitch is down.',
            ),
          );
    ...
    request
      ...
      .end(function(err, res) {
        ...
        if (!res.ok)
          return reject(
            new Error(
              'Could not access the twitch API to get the playlist, maybe your internet or twitch is down.',
            ),
          );
...
var getPlaylistOnly = function(channel) {
  ...
  return getAccessToken(channel).then(function(token) {
...
var getPlaylistParsed = function(channel) {
  return getPlaylistOnly(channel).then(function(data) {
    return Promise.resolve(_compact(M3U.parse(data)));
  });
};

var getStreamUrls = function(channel) {
  // This returns the one with a custom fully parsed object
  return getPlaylistParsed(channel).then(function(playlist) {
    if (playlist.length < 1)
      return Promise.reject(
        new Error('There were no results, maybe the channel is offline?'),
      );
